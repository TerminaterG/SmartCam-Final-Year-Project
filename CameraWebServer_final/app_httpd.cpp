...
static esp_err_t relay_handler(httpd_req_t *req) {
  ...
  digitalWrite(RELAY_PIN, HIGH);   // ON (unlock)

  vTaskDelay(7000 / portTICK_PERIOD_MS);  // 5 sec

  digitalWrite(RELAY_PIN, LOW);    // OFF (lock)

  httpd_resp_send(req, "Relay Done", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
  }
...
static esp_err_t servo_handler(httpd_req_t *req) {
  ...
  httpd_resp_send(req, "Servo OK", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &relay_uri);
